...
public class S_Pool<T> where T : class, IpoolInterface<T>
{
    ...
    public S_Pool(Func<T> createFunc, int capacity = 50, int preloadCount = 0)
        : this(createFunc, null, null, capacity, preloadCount) { }

    public S_Pool(Func<T> createFunc, Action<T> onGetFunc, Action<T> onReleaseFunc, int capacity = 50, int preloadCount = 0)
    {
